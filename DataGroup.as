...
//    [IconFile("DataGroup.png")]
    ...
    public class DataGroup extends GroupBase implements IItemRendererOwner
    {
        ...
        private function initializeTypicalItem():void
        {
            ...
            var hasRenderer:Boolean = false;
            var renderer:IVisualElement;
            if (indexToRenderer && indexToRenderer.length > 0)
            {
                renderer = indexToRenderer[0] as IVisualElement;
                hasRenderer = true;
            }
            else if (indexToRendererCache && indexToRendererCache.length > 0)
            {
                renderer = indexToRendererCache[0] as IVisualElement;
                hasRenderer = true;
            }
            else
            {
                renderer = createRendererForItem(_typicalItem, false);
            }
        ...
        override public function set layout(value:LayoutBase):void
        {
            ...
            if (value)
            {
                ...
                value.addEventListener("useVirtualLayoutChanged", layout_useVirtualLayoutChangedHandler, false, 0, true);
            }
        }
        ...
        private var dataProviderRefreshed:Boolean;
        ...
        private function removeRendererAt(index:int):void
        {
            ...
            const renderer:IVisualElement = layout && layout.useVirtualLayout ? indexToRendererCache[index] as IVisualElement : indexToRenderer[index] as IVisualElement;
            if (renderer)
            {
                var item:Object;
        ...
        {
            if (indexToRenderer.length == 0 && indexToRendererCache.length == 0)
                return;
            ...
            for (index = indexToRendererCache.length - 1; index >= 0; index--)
            {
                removeRendererAt(index);
            }

            indexToRenderer = [];  // should be redundant
            indexToRendererCache = [];  // should be redundant
        ...
        private function createItemRenderers():void
        {
            ...
            if (!layout || !layout.useVirtualLayout && !isRecyclingOK())
        ...
        override protected function commitProperties():void
        {
            ...
            if (itemRendererChanged || useVirtualLayoutChanged || dataProviderChanged || dataProviderRefreshed)
            {
                itemRendererChanged = false;
                useVirtualLayoutChanged = false;

                if (dataProviderChanged && !dataProviderRefreshed)
                {
                    removeAllItemRenderers();
                }
                dataProviderRefreshed = false;
                ...
                // validateSize() / validateDisplayList(), won't be mixed up with the
         ...
         */
        mx_internal function setUpItemRenderer(renderer:IVisualElement, itemIndex:int, data:Object):void
        {
            if (!renderer)
                return;
         ...
         */
        private var indexToRendererCache:Array = new Array();
        ...
        private function finishVirtualLayout():void
        {
            ...
            for each (var vrIndex:int in oldVirtualRendererIndices)
            {
                ...
                {
                    //see https://bugs.adobe.com/jira/browse/SDK-20962
                    //IDataRenderer(elt).data = null;
                    elt.includeInLayout = false;
                    elt.visible = false;

                    indexToRendererCache[vrIndex] = elt;
                }
                else if (elt)
                {
        ...
        {
            const freeRenderersLength:int = indexToRendererCache.length;
            for (var i:int = 0; i < freeRenderersLength; i++)
            {
                if (indexToRendererCache[i] != null)
                {
                    delete indexToRendererCache[i];
                }
            }
            indexToRendererCache.length = 0;
        }
        ...
        {
            /*            var isVirtual:Boolean = layout && layout.useVirtualLayout;
             if ((index < 0) || (isVirtual && index >= cacheRenderers.length) || (index >= indexToRenderer.length))
             return null;

             return isVirtual ? cacheRenderers[index] : indexToRenderer[index];
             */
        ...
        override public function getVirtualElementAt(index:int, eltWidth:Number = NaN, eltHeight:Number = NaN):IVisualElement
        {
            ...
            if (virtualLayoutUnderway)
            {
                ...
                var hasRenderer:Boolean = elt != null;
                if (!elt)
                {
                    hasRenderer = isRecyclingOK() && indexToRendererCache && indexToRendererCache[index] != null;
                    if (hasRenderer && (itemRendererFunction == null || IItemRenderer(indexToRendererCache[index]).data == item))
                    {
                        elt = indexToRendererCache[index];
                        elt.visible = true;
                    ...
                    else
                    {
                        ...
                        createdIR = true;
                        indexToRendererCache[index] = elt;
                    }
                ...
                if (elt is IItemRenderer && (createdIR
                        || IItemRenderer(elt).itemIndex != index || IItemRenderer(elt).data != item))
                {
                ...
                //if (createdIR && elt is ILayoutManagerClient)
        ...
        private function resetRendererItemIndex(index:int):void
        {
            ...
            var renderer:IItemRenderer = indexToRendererCache[index] as IItemRenderer;
            if (renderer)
        ...
        mx_internal function itemAdded(item:Object, index:int):void
        {
            ...
            if (layout && layout.useVirtualLayout)
            {
                ...
                if (virtualRendererIndices)
                {
                    ...
                    indexToRendererCache.splice(index, 0, null); // shift items >= index to the right
            ...
            indexToRendererCache.splice(index, 0, myItemRenderer);
        ...
        mx_internal function itemRemoved(item:Object, index:int):void
        {
            ...
            const oldRenderer:IVisualElement = layout && layout.useVirtualLayout ? indexToRendererCache[index] : indexToRenderer[index];

            if (oldRenderer is IDataRenderer && oldRenderer !== item)
                IDataRenderer(oldRenderer).data = null;

            if (indexToRendererCache.length > index)
            {
                indexToRendererCache.splice(index, 1);
            }
                ...
                case CollectionEventKind.UPDATE:
                {
                    ...
                    for (var i:int = 0; i < event.items.length; i++)
                    {
                        ...
                        if (pe)
                        {
                            ...
                            var renderer:IVisualElement = layout && layout.useVirtualLayout ? indexToRendererCache[index] : indexToRenderer[index];
        ...
        mx_internal function adjustAfterReplace(propertyChangeEvents:Array, location:int):void
        {
            /*            var length:int = items.length;
             for (var i:int = length - 1; i >= 0; i--)
             {
            ...
            // The version of this method that follows is relatively efficient.
            // See http://bugs.adobe.com/jira/browse/SDK-29819
            var itemIndex:int = location;
            for each (var pce:PropertyChangeEvent in propertyChangeEvents)
            {
                var renderer:IVisualElement = getElementAt(itemIndex);
                if (renderer)
                    setUpItemRenderer(renderer, itemIndex, pce.newValue);
                itemIndex += 1;
            }
         ...
         *  @private
         */
        private function invalidateSizeAndDisplayList():void
        {
            invalidateSize();
            invalidateDisplayList();
        }
